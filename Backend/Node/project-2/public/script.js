...
function renderPostList() {
	...
	posts.forEach(post => {
        ...
        <p class="descrip">${post.body}</p>
				<div class="butContainer">
					<button onclick="editPost(${post.id})" class="button">
						<span class="button-content">Edit</span>
					</button>
					<button onclick="deletePost(${post.id})" class="button">
						<span class="button-content">Delete</span>
					</button>
				</div>

        <div id="editForm-${post.id}" class="editForm" style="display:none">
            <input class="input" type="text" id="editTitle-${post.id}" value="${post.title}" required>
            <textarea class="input" id="editBody-${post.id}" required>${post.body}</textarea>
						<button onclick="deletePost(${post.id})" class="button">
							<span onclick="updatePost(${post.id})" class="button-content">Update</span>
						</button>
        </div>
        `
